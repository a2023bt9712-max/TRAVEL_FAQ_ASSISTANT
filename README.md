...
Travel guides are stored as text documents (Python dictionary).
Documents are embedded using:
all-MiniLM-L6-v2
FAISS stores these embeddings in a vector index.
User query is embedded and searched in FAISS.
Top result (k=1) is retrieved as context.
FLAN-T5 generates the itinerary using only that context.
🛠️ Tech Stack
Purpose	Technology
